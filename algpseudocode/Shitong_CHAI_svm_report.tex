...
\end{theorem}
\begin{proof}
    Use Theorem 1.1.2 to proof. Notice that this theorem is a special case of Theorem 1.1.2 where $\Lambda(u)\in \mathcal L(\mathbb R^m;\mathbb R)$, such that the equation
    \[
        dJ(u)+\Lambda(u)\circ d\varphi(u)=0
    \]
    can be rewritten as 
...
\begin{definition}
    (Hermitian form\cite{gallier2019algebra}) If a function $\varphi :E\times E\to \mathbb C$ is sesquilinear and $\forall u,v\in E$, we have $\varphi(v,u)=\overline{\varphi(u,v)}$, then it is a Hermitian form.
\end{definition}
\begin{definition}
    (Hermitian Space\cite{gallier2019algebra}) Given a complex vector space E, a Hermitian form $\varphi:E\times E\to \mathbb C$ is positive of $\varphi(u,u)\geq 0$ for all $u\in E$, and positive definite if $\varphi(u,u)>0$ for all $u\neq 0$. A pair $\langle E,\varphi \rangle$ where E is a complex vector space and $\varphi$ is a Hermitian form on E is called a pre-Hilbert space if $\varphi$ is positive, and a Hermitian space if $\varphi$ is positive definite.
\end{definition}
...
\begin{theorem}
    (Farkas-Minkowski Lemma\cite{gallier2019algebra}) Let $(V,\langle -,- \rangle)$ be a real Hilbert space. For any finite sequence of vectors $(a_1,\cdots,a_m)$ with $a_i\in V$, if C is the polyhedral cone $C=\mathrm{cone}(a_1,\cdots,a_m)$, for any vector $b\in V$, we have $b\notin C$ iff there is a vector $u\in V$ such that
    \[
        \langle a_i, u\rangle \geq 0 \; i=1,\cdots,m, \; and \; \langle b, u \rangle <0.
    \]
    Equivalently, $b\in C$ if for all $u\in V$,
    \[
        if \, \langle a_i,u\rangle \geq 0 \; i=1,\cdots,m,\; then \langle b,u\rangle \geq 0.
    \]
\end{theorem}
